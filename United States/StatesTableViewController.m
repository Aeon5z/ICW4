...
- (void)viewDidLoad {
    ...
    //file path
    self.pListPath = [[NSBundle mainBundle]pathForResource:@"StateList" ofType:@"plist"];
    
    self.unitedStates = [[NSDictionary alloc] initWithContentsOfFile:self.pListPath];
   
    
    NSLog(@"%@" , self.unitedStates[@"Alabama"]);
    
    
    self.tableView.backgroundView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"UnitedStatesBG"]];
    [super viewDidLoad];
    
    self.keys = [[self.unitedStates allKeys]sortedArrayUsingSelector:@selector(compare:)];
   ...
   //  self.usStates = [NSMutableArray arrayWithObjects:Alabama, nil];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return  [self.unitedStates count];
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
     NSString *cellIdentifier = @"StateCellIdentifier";
    StatesTableViewCell *cell = [ tableView dequeueReusableCellWithIdentifier:cellIdentifier forIndexPath:indexPath];
    //populate
    NSString *key = [self.keys objectAtIndex:indexPath.row];
    cell.StateName.text = self.unitedStates[key][@"StateName"];
    cell.StateMoto.text = self.unitedStates[key][@"Motto"];
    cell.Flagimage.image = [UIImage imageNamed:self.unitedStates[key][@"Flag"]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier]isEqualToString:@"showDetails"])  {
        ...
        StateInfo *item = [[StateInfo alloc] init];
        NSString *key = [self.keys objectAtIndex:myIndexPath.row];
        

        // data population, (preparing to pas)
        /*
         ...
         */
        item.name = self.unitedStates[key][@"StateName"];
        item.motto = self.unitedStates[key][@"Motto"];
        item.capital = self.unitedStates[key][@"Capital"];
        item.population = self.unitedStates[key][@"Population"];
        item.birdname = self.unitedStates[key][@"Year"];
        item.flag =  [UIImage imageNamed:self.unitedStates[key][@"Flag"]];
